...
| `route.overflow-scroll` | Help, Project, or Session list with overflow; wheel or scrollbar click/drag | owning Help/list viewport; terminal focus consumes list wheel as a no-op | viewport stays within first/last row; selection changes only through navigation input | async output and resize repaint preserve the current scroll owner | no row activation, project/session switch, or PTY input from scrollbar gestures | `TestWorkspaceScrollbarGeometryTracksOverflowAndBounds`, `TestWorkspaceScrollbarTrackPagesThumbDragAndRelease`, `TestHelpScrollMouseDispatcherWheelTrackDragAndRelease` (geometry/dispatcher); `TestDucklordScrollContainerE2E` (default E2E; current-cell scrollbar and PageUp/PageDown proof) |
