...
| `route.overflow-scroll` | Help, Project, or Session list with overflow; PageUp/PageDown, wheel or scrollbar click/drag | owning Help/list viewport; terminal focus consumes list wheel as a no-op | viewport stays within first/last row; list gestures move the navigation selection and corresponding terminal preview | async output and resize repaint preserve the current scroll owner | navigation focus remains in the list; no terminal focus, writer-control acquisition, or PTY input from scrollbar gestures | `TestWorkspaceScrollbarGeometryTracksOverflowAndBounds`, `TestWorkspaceScrollbarTrackPagesThumbDragAndRelease`, `TestHelpScrollMouseDispatcherWheelTrackDragAndRelease`, `TestWorkspaceProjectPageDownPagesProjectListBeforeTabShortcuts`, `TestProjectTabShortcutDefaultsAvoidListPagingKeys` (geometry/dispatcher); `TestDucklordScrollContainerE2E` (default E2E; current-cell scrollbar and PageUp/PageDown proof) |
