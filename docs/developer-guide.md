...
When a human posts in a task channel, the gateway forwards the event over SSE to the on-machine `duckway cc watch` daemon. For `claude_code`, the daemon runs `claude --print --output-format json` inside Duckway's PTY supervisor by default. For `codex`, it runs `codex exec --json` inside Duckway's PTY supervisor by default; follow-up turns use `codex exec resume <thread_id>`. Safe semantic Codex events update one Discord progress message at most every five seconds; raw prompts, command output, paths, and errors are never copied into that preview. The Codex thread ID is persisted as soon as `thread.started` arrives, so a later transport hang does not split the conversation. The legacy tmux runner is still available with `duckway cc watch --tmux` or `DUCKWAY_CC_USE_TMUX=1`. For `openclaw`, it runs `openclaw agent --agent <id> --session-key duckway:<handle> --message-file <file> --json`; the agent id comes from `DUCKWAY_CC_OPENCLAW_AGENT` or defaults to `default`, and Duckway does not use OpenClaw's own channel bindings. The daemon posts the final agent message back to the channel.

PTY execution uses an output-idle timeout rather than the old five-minute wall-clock limit. `DUCKWAY_CC_PTY_IDLE_TIMEOUT` defaults to `30m`; set a Go duration such as `90m`, or `0`/`off`/`disabled` to disable only the idle limit. `DUCKWAY_CC_PTY_MAX_RUNTIME` defaults to `6h` and is always enforced, with configured values capped at `24h`. Restart `duckway cc watch` after changing either environment variable.
...
| `discord_inbox` | Durable Discord ingress jobs. Gateway replay is deduplicated by typed event key. When cc-watch is connected, daemon-bound `!` commands are inserted synchronously using their message snowflake before Gateway dispatch returns; when it is offline, commands fail immediately without admission. Clients atomically lease per-channel lane heads and finish them as completed, retryable, or dead-letter. |
