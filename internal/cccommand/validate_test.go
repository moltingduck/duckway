...
func TestValidateCommandArguments(t *testing.T) {
	tests := []struct {
		...
		{name: "yield immediate", command: "!yield"},
		{name: "yield wait short", command: "!yield", args: []string{"-w"}},
		{name: "yield wait long", command: "!yield", args: []string{"--wait"}},
		{name: "yield unknown flag", command: "!yield", args: []string{"--force"}, wantErr: true},
		{name: "end immediate", command: "!end"},
		{name: "end wait short", command: "!end", args: []string{"-w"}},
		{name: "end force long", command: "!end", args: []string{"--force"}},
		{name: "end conflicting modes", command: "!end", args: []string{"--wait", "--force"}, wantErr: true},
		{name: "destroy immediate", command: "!destroy"},
		{name: "destroy wait long", command: "!destroy", args: []string{"--wait"}},
		{name: "destroy force short", command: "!destroy", args: []string{"-f"}},
		{name: "destroy unknown mode", command: "!destroy", args: []string{"--later"}, wantErr: true},
		{name: "restart waits by default", command: "!restart"},
		{name: "restart force", command: "!restart", args: []string{"-f"}},
		{name: "restart explicit wait unsupported", command: "!restart", args: []string{"--wait"}, wantErr: true},
