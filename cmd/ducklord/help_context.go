...
func helpCatalog(_ bool) []helpEntry {
	entries := []helpEntry{
		{"SESSION LIST & GROUPS", "list_search", "Search sessions", "filter the visible list"},
		{"", "list_organize", "Cycle list organization", "custom / host / type"},
		{"", "list_groups", "Manage custom groups", "custom organization only; create or rename groups, move the selected session between groups, delete groups, or move groups up/down; arrows or j/k choose and Enter selects; q/Esc closes"},
		{"", "list_reorder_up", "Move session up", "custom ordering"},
		{"", "list_reorder_down", "Move session down", "custom ordering"},
		...
		{"", "project_detach_pane", "Detach Session pane", "Default Project asks for confirmation"},
		...
		{"", "prefix+-", "Create horizontal Session pane", "one press opens creation flow; doubled suffix creates a quick shell when focused on a live Session"},
		{"", "prefix+\\", "Create vertical Session pane", "one press opens creation flow; doubled suffix creates a quick shell when focused on a live Session"},
		...
		{"", "prefix+--", "Create horizontal quick-shell pane", "press the pane prefix and - twice within 500 ms on a focused live Session"},
		{"", "prefix+\\\\", "Create vertical quick-shell pane", "press the pane prefix and \\ twice within 500 ms on a focused live Session"},
		{"", "prefix+tt", "Create quick-shell Terminal tab", "press the pane prefix and t twice within 500 ms on a focused live Session"},
		{"", "prefix+,", "Rename Terminal tab", ""},
		{"", "prefix+c", "Configure focused item or pane", "right-click also opens configuration"},
		{"", "prefix+d", "Detach focused Session pane", "Default Project confirms; other Projects detach directly"},
		...
		{"", "", "Project configuration menu", "the configurable workspace-menu shortcut opens Create/Rename Project, Project Hosts, Export/Import, Workspace appearance, and shortcut settings"},
		...
		{"PROJECT FILES", "", "Browse Project files", "Tab switches columns; arrows/j/k select; Enter opens; Backspace/Delete goes to parent; Esc closes; Ctrl+C cancels; h opens endpoint picker; g changes path; / filters; Space marks; i toggles folder icons; c previews copy; l opens history"},
		...
		{"", "", "Import Project", "Project configuration offers Import; validate and preview before applying; Enter confirms; Esc cancels; collisions require confirmation; no remote Sessions are created"},
		{"HOST SKILLS", "", "Host Skills dashboard", "Tab/Left/Right switches panes; arrows/j/k select; Enter/Space expands. Selected skill: p push, n none, m rename, i import, a add source, f fetch, t target, r pull. Target pane: t target; remote row r pulls and x deletes; d deploys"},
