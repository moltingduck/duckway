...
func TestDucklordShellRetirementContainerE2E(t *testing.T) {
	...
	owner := fmt.Sprintf("retire-owner-%x", time.Now().UnixNano()&0xffffff)
	marker := "DUCKLORD_RETAINED_E2E_" + handle
	args := []string{"exec", controller, "ducklord", "--name", owner, "start", "client-a", "--name", handle, "--kind", "shell", "--cwd", "/home/duck", "--config", "/root/.ducklord/config.yaml", "--", "sh"}
	...
	if out, err := exec.Command(runtime, "exec", controller, "ducklord", "--name", owner, "send", "client-a", sessionID,
	...
	retained, err := exec.Command(runtime, "exec", controller, "ducklord", "--name", owner, "retained", "client-a", "--config", "/root/.ducklord/config.yaml").CombinedOutput()
	...
	read, err := exec.Command(runtime, "exec", controller, "ducklord", "--name", owner, "read-retained", "client-a", sessionID,
...
func TestDucklordExplicitShellEndRetainsLogContainerE2E(t *testing.T) {
	...
	owner := fmt.Sprintf("explicit-end-owner-%x", time.Now().UnixNano()&0xffffff)
	...
	waitE2E(t, 10*time.Second, func() bool {
		out, err := exec.Command(runtime, "exec", controller, "ducklord", "--name", owner, "read", "client-a", sessionID,
	...
	if out, err := exec.Command(runtime, "exec", controller, "ducklord", "--name", owner, "end", "client-a", sessionID,
		"--config", "/root/.ducklord/config.yaml").CombinedOutput(); err != nil {
